...
Import all count matrixes (from data/Seq2Science folder), combine plates and create unfiltered Seurat objects. 

```{r convert_matrix_to_seurat}
...
2. Filter low-detected genes

Keep genes that are >1% cells detected.

```{r filter non-detected genes, class.source = 'fold-show'}
...
```{r merge.protRNA.data, class.source = 'fold-show'}
...
4. Antibody quality (Non-detected proteins) filter 

```{r Functions_protein_plot}
...
```{r p.suppl_Ab-QC_fix, fig.width=7.2, fig.height=7.2}
ggplot(PROT_tbl_subset.fix, aes(reorder_within(protein, count, sample), count)) +
  ...
  add.textsize +
  coord_flip() +
  labs(y="count (log10)", x = "Antibody (ordered per plate)")
```
...
```{r p.suppl_Ab-QC_live, fig.width=7.2, fig.height=8.5}
ggplot(PROT_tbl_subset.live, aes(reorder_within(protein, count, sample), count)) +
     geom_boxplot(outlier.colour="black", outlier.shape=16,
             outlier.size=0.5, notch=FALSE)  +
     scale_x_reordered() +
     facet_wrap(.~ sample, ncol=4,scales = "free_y") + 
...
5. Add metadata to object:

```{r add.metadata, class.source = 'fold-show'}
## metadata import
...
```{r Normalize_and_scale, class.source = 'fold-show'}
